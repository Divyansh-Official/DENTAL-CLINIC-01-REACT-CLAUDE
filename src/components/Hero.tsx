...
// Derive a human-readable hours string from clinic.json timings.
// Groups consecutive days with the same open/close window.
function buildHoursLabel(): string {
  ...
  // Find the dominant open window (first open day)
  const firstOpen = order.find((d) => clinic.timings[d].is_open);
  if (!firstOpen) return "Closed";

  const { open, close } = clinic.timings[firstOpen];
  const openDays = order.filter((d) => {
    const t = clinic.timings[d];
    return t.is_open && t.open === open && t.close === close;
  });
  ...
  const indices = openDays.map((d) => order.indexOf(d));
  const isRange =
    indices.length > 1 &&
    indices[indices.length - 1] - indices[0] === indices.length - 1;

  const dayStr = isRange
    ? `${abbr[openDays[0]]} – ${abbr[openDays[openDays.length - 1]]}`
    : openDays.map((d) => abbr[d]).join(", ");
...
export default function Hero({ onBooking }: HeroProps) {
  ...
  // Derived values from clinic.json
  const hoursLabel = buildHoursLabel();
  const shortAddress = `${clinic.location.address.split(",").slice(-3, -1).join(",").trim()}, ${clinic.location.city}`;
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-6 md:px-12 pt-28 pb-12">
        ...
        {/* Badge — uses established year from clinic.json */}
        <div
        ...
        >
          ...
          Holistic Dental Care · Est. {clinic.established}
        </div>
        ...
        {/* Description — from clinic.json about */}
        <p
      ...
      {/* Bottom info bar — hours and address from clinic.json */}
      <div
        className="
          relative z-10 px-6 md:px-2 pb-10
          flex flex-col sm:flex-row gap-6 sm:gap-16
          lg:absolute lg:right-[650px] lg:bottom-1
      ...
      >
        <div>
          ...
          <div className="text-[14px] font-semibold text-white">{hoursLabel}</div>
        </div>
        ...
        <div>
          ...
          <div className="text-[14px] font-semibold text-white">{shortAddress}</div>
        </div>
      </div>
      ...
      <div className="absolute right-8 bottom-10 hidden lg:flex flex-col items-center gap-2 z-10">
        <div className="w-px h-16 bg-white/20 relative overflow-hidden">
          <div
            ...
            style={{ animation: "scrollLine 1.8s ease infinite", height: "40%" }}
          />
        </div>
