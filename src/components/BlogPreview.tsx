...
function ImagePlaceholder({ category }: { category: string }) {
  return (
    <div className="w-full h-full bg-gradient-to-br from-[#F0F0F0] to-[#E0E0E0] flex items-center justify-center">
      <span className="text-[11px] font-bold tracking-[0.15em] uppercase text-[#C0C0C0]">
        {category}
      </span>
    </div>
  );
      ...
      {!loading && !error && featured && (
        <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-0.5">
          ...
          <Link
            ...
            className="group block bg-[#F5F5F5] hover:bg-[#EBEBEB] transition-colors duration-300 flex flex-col"
          >
            {/* Cover image */}
            <div className="relative w-full h-52 md:h-64 overflow-hidden flex-shrink-0">
              {featured.imageUrl ? (
                <img
                ...
                <ImagePlaceholder category={featured.category} />
              )}
              {/* Category badge over image */}
              <span className="absolute top-4 left-4 flex items-center gap-1.5 bg-white/90 backdrop-blur-sm text-[10px] font-bold tracking-[0.15em] uppercase text-[#555] px-2.5 py-1 rounded-full">
...
function BlogSmallCard({ post, index }: { post: BlogPost; index: number }) {
  ...
  return (
    <Link
      ref={ref}
      to={`/blog/${post.slug}`}
      className="section-reveal group flex items-center gap-4 bg-[#F5F5F5] hover:bg-[#EBEBEB] transition-colors duration-200 px-5 md:px-7 py-4 overflow-hidden"
    >
      {/* Thumbnail */}
      <div className="w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 bg-[#E0E0E0]">
        {post.imageUrl ? (
          <img
            src={post.imageUrl}
            alt={post.title}
      ...
      <div className="flex-1 min-w-0">
        ...
        <h4 className="text-[14px] font-bold tracking-[-0.01em] text-black leading-snug line-clamp-2">
          {post.title}
        </h4>
