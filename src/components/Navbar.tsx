...
// Split "Surya Dental Care and Implant Clinic" → primary + sub
const [primaryName, ...rest] = clinic.clinic_name.split(" and ");
const subName = rest.join(" and ");

interface NavbarProps { onBooking: () => void; }
...
export default function Navbar({ onBooking }: NavbarProps) {
  ...
  useEffect(() => {
    fetchClinicPhoto(clinic.google_place_id).then(setClinicPhoto);
  }, []);
  ...
  // Initials fallback from clinic name
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-[100] flex items-center justify-between transition-all duration-400 ${
        scrolled
        ...
        <a href="#" className="flex items-center gap-3 select-none">
          {clinicPhoto ? (
            <img src={clinicPhoto} alt={clinic.clinic_name}
        ...
        {/* Hamburger */}
        <button className="md:hidden text-white p-2" onClick={() => setMobileOpen(p => !p)} aria-label="Toggle menu">
          {mobileOpen ? <X size={22} /> : <Menu size={22} />}
        </button>
      </nav>

      {/* Mobile drawer */}
      <div className={`fixed inset-0 z-[99] bg-black flex flex-col items-center justify-center gap-8 transition-all duration-500 md:hidden ${
        ...
        <div className="flex flex-col items-center gap-2 mb-2">
          ...
          <p className="text-white/40 text-xs tracking-widest uppercase">{clinic.location.city}</p>
        </div>

        {NAV_LINKS.map((l) => (
          <button key={l.label} onClick={() => scrollTo(l.href)}
            className="text-3xl font-black tracking-[-0.03em] text-white/80 hover:text-white transition-colors">
            {l.label}
          </button>
        ))}
        ...
        <a href={`tel:${clinic.contact.phone.replace(/\s/g, "")}`} className="text-white/50 text-sm">
          {clinic.contact.phone_display}
        </a>
      </div>
    </>
