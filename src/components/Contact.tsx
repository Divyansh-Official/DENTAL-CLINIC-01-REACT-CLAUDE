...
// Build contact items directly from clinic.json
const CONTACT_ITEMS = [
  { Icon: Phone,         label: "Phone",    value: clinic.contact.phone_display,  sub: "Mon–Sat, 10 AM – 7:30 PM" },
  { Icon: MapPin,        label: "Location", value: clinic.location.address,        sub: clinic.location.landmark },
  { Icon: Mail,          label: "Email",    value: clinic.contact.email,           sub: "Reply within 2 hours" },
  { Icon: MessageCircle, label: "WhatsApp", value: clinic.contact.phone_display,   sub: "Share reports & quick queries" },
];

// Build hours from clinic.json timings
...
const { timings } = clinic;
const weekdayOpen  = timings.monday.is_open;
const weekdayRange = weekdayOpen
  ? `${formatTime(timings.monday.open!)} – ${formatTime(timings.monday.close!)}`
  : "Closed";
const satRange = timings.saturday.is_open
  ? `${formatTime(timings.saturday.open!)} – ${formatTime(timings.saturday.close!)}`
  : "Closed";
const sunRange = timings.sunday.is_open
  ? `${formatTime(timings.sunday.open!)} – ${formatTime(timings.sunday.close!)}`
  : "Closed";

const HOURS = [
  { day: "Mon – Fri", time: weekdayRange },
  { day: "Saturday",  time: satRange },
  { day: "Sunday",    time: sunRange },
  { day: "Emergency", time: "24 / 7 Hotline" },
];

const mapSrc = clinic.location.lat && clinic.location.lng
  ? `https://www.google.com/maps?q=${clinic.location.lat},${clinic.location.lng}&z=15&output=embed`
  : "";
...
export default function Contact() {
  ...
  <a
    ...
    className="text-[12px] font-semibold text-white hover:text-white/80 transition whitespace-nowrap"
  >
    Open in Maps →
  </a>
</div>
